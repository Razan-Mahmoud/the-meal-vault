...
export default async function MealsListPage({ id, fetcher }: MealsListPageProps) {
  ...
  return (
    <section
      id={id}
      className="grid grid-cols-1 items-stretch gap-6 p-4 md:grid-cols-2 lg:grid-cols-4 dark:bg-slate-700"
    >
      {payload.meals?.map((meal) => {
        return (
          ...
          >
            ...
            <p className="pt-4 text-center text-lg font-semibold">{meal.strMeal}</p>
          </Link>
        );
      })}
